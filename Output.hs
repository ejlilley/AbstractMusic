...
import Music (Note3, AbstractNote(..), AbstractPitch3(..), AbstractInt3(..), AbstractDur3(..), absolute, AbstractPhrase(..), phraseToList)
import Util (log2)
...
playCsounds notes = do writeFile "./csound-test.sco" $ csounds notes;
                       readProcess "csound" ["-d", "csound-test/organ.orc", "csound-test.sco"] "";
                       system "mplayer -volume 48 test.wav"
